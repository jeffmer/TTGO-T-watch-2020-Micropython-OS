...
class ValueDisplay:
    ...
    def __init__(self,title,Y,barorval,incr,userfn,buttonman):
        ...
        self._incr = incr
        self._userfn = userfn
        self._minus = ArrowButton("-",0,self._YOFF+_Y,50,_H,theme=self.theme,dir=3)
        self._plus = ArrowButton("+",_X+_W,self._YOFF+_Y,50,_H,theme=self.theme,dir=1)
    ...
    def drawBar(self,v,now=True):
        ...
        g.fill_rect(_X+_B+2,self._YOFF+_Y+_B+2,math.ceil((_W-2*_B-4)*v),_H-_B*2-4,YELLOW)
        if now:
            g.show()
...
class SwitchPanel(Button):
    ...
    def _drawswitch(self,x,y,w,h,state):
        ...
        g.ellipse(x+w-r if state else x+r,y+r,r-2,r-2,self.theme._bg,True)
