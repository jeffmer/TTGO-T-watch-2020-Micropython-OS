...
class ST7789(graphics.Graphics):
    ...
    def __init__(self, spi, cs, dc, bl, rst=None, height=240, width=240, usd=False):
        self._spi = spi
        self._cs = cs
        self._dc = dc
        self._bl = PWM(bl)
    ...
    def show(self, modonly=True):
        ...
        if not modonly or (ye - ys) > (self.height // 2):
            self._wcd(
                b"\x2a", int.to_bytes(self.width + XOFF - 1 + (XOFF << 16), 4, "big")
            )  # SET_COLUMN
            self._wcd(
                b"\x2b", int.to_bytes(self.height + YOFF - 1 + (YOFF << 16), 4, "big")
            )  # SET_PAGE
